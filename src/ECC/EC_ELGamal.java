...
public class EC_ELGamal {
    ...
    /**
     * @param P duong cong eliptic P
     * @param C tap 2 diem ban ma (M1,M2)
     * @param s so bi mat s
     * @return ban ro
     */
    public static BigInteger ElGamalDec(PrimeCurve P, Point[] C, BigInteger s) {
        Point M1 = C[0];
        Point M2 = C[1];
        Point temp = P.mul(M1, s);
        Point M = P.sub(M2, temp); // M = M2 - sM1
        return EC_ELGamal.KoblitzDec(P, M);
    }
